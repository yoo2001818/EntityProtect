...
public class BreedChecker {
	...
	public static boolean pigCheck(EntityType entity, Material item) {
		return entity.equals(EntityType.PIG)
				&& item.equals(Material.CARROT_ITEM);
	}

	public static boolean chickenCheck(EntityType entity, Material item) {
		if (!entity.equals(EntityType.CHICKEN))
			return false;
		if (item.equals(Material.SEEDS))
			return true;
		if (item.equals(Material.MELON_SEEDS))
			return true;
		if (item.equals(Material.PUMPKIN_SEEDS))
			return true;
		if (item.equals(Material.NETHER_WARTS))
			return true;
		return false;
	}
	...
	public static boolean mushroomcowCheck(EntityType entity, Material item) {
		return entity.equals(EntityType.MUSHROOM_COW)
				&& item.equals(Material.WHEAT);
	}

	public static boolean ozelotCheck(EntityType entity, Material item) {
		return entity.equals(EntityType.OCELOT)
				&& item.equals(Material.RAW_FISH);
	}

	public static boolean check(EntityType entity, Material item) {
		if (wolfCheck(entity, item))
			return true;
		if (pigCheck(entity, item))
			return true;
		if (cowCheck(entity, item))
			return true;
		if (sheepCheck(entity, item))
			return true;
		if (mushroomcowCheck(entity, item))
			return true;
